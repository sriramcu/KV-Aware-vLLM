#!/usr/bin/env python3
"""Foreground, resumable Slurm runner for the SC LMCache future-combination grid."""
...
GRID_VERSION = "sc-future-combo-grid-v1"
SCHEMA_VERSION = 1
...
def base_environment(
    ...
    if lmcache_data_dir is not None:
        # Jobs in this grid run sequentially. Reusing one dedicated cache
        # directory is intentional: run_driver.sh deletes/recreates it at the
        # start of every job, so scratch usage does not accumulate.
...
def make_grid(
    ...
    # The meeting slide's experiments 1 and 4 are identical under the current
    # defaults. Experiment 4 is made unique here by adding worker admission=4.
    rows: list[tuple[str, str, dict[str, str]]] = [
        (
            "00_best_combo",
            (
                "Best observed throughput + pressure-control combination: "
                "scheduler admission max 4 wait 0; disk-put admission disabled; "
                "max_num_seqs 12; batch 50"
            ),
            {
        ...
        (
            "01_scheduler_plus_worker",
            (
                "Scheduler admission max 4 wait 0 plus worker admission max 4; "
                "baseline vLLM concurrency and batch size"
            ),
            {
                "SC_LMCACHE_SCHEDULER_LOOKUP_ADMISSION_ENABLE": "1",
                "SC_LMCACHE_SCHEDULER_LOOKUP_MAX_INFLIGHT": "4",
        ...
        (
            "02_best_combo_batch_250",
            (
                "Throughput-oriented stress test: scheduler admission max 4 wait 0; "
                "max_num_seqs 12; submission batch 250; disk-put admission disabled"
            ),
            {
                "SC_LMCACHE_SCHEDULER_LOOKUP_ADMISSION_ENABLE": "1",
                "SC_LMCACHE_SCHEDULER_LOOKUP_MAX_INFLIGHT": "4",
        ...
        (
            "03_full_read_control_plus_throughput",
            (
                "Conservative combined setting: scheduler admission max 4 wait 0; "
                "worker admission max 4; max_num_seqs 12; batch 50; "
                "disk-put admission disabled"
            ),
            {
                "SC_LMCACHE_SCHEDULER_LOOKUP_ADMISSION_ENABLE": "1",
                "SC_LMCACHE_SCHEDULER_LOOKUP_MAX_INFLIGHT": "4",
        ...
        (
            "04_best_combo_hotpotqa",
            (
                "Repeat the best combo on HotpotQA to test workload sensitivity: "
                "scheduler admission max 4 wait 0; max_num_seqs 12; batch 50; "
                "disk-put admission disabled"
            ),
            {
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "/scratch/$USER/lmcache_future_grid. run_driver.sh clears it "
    ...
    parser.add_argument(
        "--print-grid",
        action="store_true",
        help="Print the future-combination configurations and exit without creating state or submitting jobs.",
    )
...
def main() -> int:
    ...
    sbatch_script = script_path.with_name("submit_future_combo_grid.sbatch")
    ...
    try:
        while True:
            ...
            if index is None:
                ...
                print(f"All {len(grid)} future-combination configurations are terminal.", flush=True)
