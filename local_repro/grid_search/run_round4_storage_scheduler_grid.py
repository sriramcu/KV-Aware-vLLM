#!/usr/bin/env python3
"""Foreground, resumable Slurm runner for Round 4: storage x scheduler width.

Cells:
  A = shared NFS, scheduler max inflight 4
  B = shared NFS, scheduler max inflight 12
  C = node-local ext4 scratch, scheduler max inflight 4
  D = node-local ext4 scratch, scheduler max inflight 12

Frozen by default: Medical, Q=50, batch=250, CPU LMCache=20 GiB/rank,
max_num_seqs=16, GNN off, scheduler wait=0, worker gate off, disk-put gate off.
"""
...
GRID_VERSION = "sc-round4-storage-scheduler-v1-cpu20"
SCHEMA_VERSION = 1
...
def base_environment(max_questions: int, nodelist: str) -> dict[str, str]:
    """Frozen Round-4 environment; only storage and scheduler width vary."""
    return {
        ...
        # Explicitly reduce the CPU tier so Q=50 must exercise disk reuse.
        ...
        "CLEAN_OLD_LMCACHE": "1",
        # Captured in the config hash so resume cannot silently move nodes.
        "SC_ROUND4_NODELIST": nodelist,
    }
...
def make_grid(
    ...
    rows: list[tuple[str, str, dict[str, str]]] = [
        ...
        (
            ...
            "C: node-local ext4 scratch, scheduler max inflight 4",
            {
                "ROUND4_STORAGE": "node_local_ext4",
                "SC_LMCACHE_SCHEDULER_LOOKUP_MAX_INFLIGHT": "4",
...
def write_progress(
    ...
    lines = [
        "SC LMCache Round-4 storage x scheduler grid",
    ...
    if current_index is None:
        ...
        lines.extend(
            [
                ...
                f"Storage: {env['ROUND4_STORAGE']}",
                f"Node: {env['SC_ROUND4_NODELIST']}",
...
def submit_run(
    *,
    ...
    with deferred:
        ...
        nodelist = str(config["environment"]["SC_ROUND4_NODELIST"])
        command = [
            "sbatch",
            "--parsable",
            f"--job-name={job_name}",
...
def print_grid(grid: list[dict[str, Any]]) -> None:
    for config in grid:
        ...
        print(
            ...
            f"    storage={env['ROUND4_STORAGE']} node={env['SC_ROUND4_NODELIST']} "
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        type=int,
        default=50,
        help="Questions per cell. Defaults to 50 for the actual Round-4 experiment.",
    )
    parser.add_argument(
        "--state-dir",
        type=Path,
    ...
    parser.add_argument(
        "--scratch-dir",
        type=Path,
        default=Path("/scratch/sriramc2/lmcache_med_sched4_seq16_q50"),
        help=(
            "Dedicated disposable node-local LMCache directory for cells C/D. "
    ...
    parser.add_argument(
        ...
        help=(
            "Pin all four cells to the same H100 compute node. Required for a "
    ...
    parser.add_argument(
        "--print-grid",
        action="store_true",
        help="Print the four Round-4 cells and exit without creating state or submitting jobs.",
    )
...
def validate_driver_cpu_override(repo: Path) -> None:
    ...
    if "SC_LMCACHE_MAX_LOCAL_CPU_SIZE" not in text:
        raise SystemExit(
            "Round 4 requires the driver to honor SC_LMCACHE_MAX_LOCAL_CPU_SIZE. "
...
def main() -> int:
    ...
    sbatch_script = script_path.with_name("submit_round4_storage_scheduler_grid.sbatch")
    ...
    try:
        while True:
            ...
            if index is None:
                ...
                print(f"All {len(grid)} Round-4 storage/scheduler cells are terminal.", flush=True)
